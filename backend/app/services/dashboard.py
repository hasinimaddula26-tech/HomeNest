...
def get_dashboard_metrics(db: Session) -> Dict[str, Any]:
    ...
    # Current month sum
    current_month_sum = db.query(func.sum(Expense.amount)).filter(Expense.date >= current_month_start).scalar() or Decimal('0.00')
    ...
    # Groceries
    groceries_total = db.query(func.count(Grocery.id)).scalar() or 0
    groceries_completed = db.query(func.count(Grocery.id)).filter(Grocery.is_completed == True).scalar() or 0

    # Bills
    pending_bills_count = db.query(func.count(Bill.id)).filter(Bill.is_paid == False).scalar() or 0

    # Reminders
    active_reminders_count = db.query(func.count(Reminder.id)).filter(Reminder.is_completed == False).scalar() or 0
...
def get_upcoming_bills(db: Session, limit: int = 5) -> List[Dict[str, Any]]:
    today_val = date.today()
    bills = db.query(Bill).filter(Bill.is_paid == False).order_by(Bill.due_date.asc()).limit(limit).all()
...
def get_today_reminders(db: Session) -> List[Reminder]:
    today_val = date.today()
    return db.query(Reminder).filter(
        Reminder.date == today_val
    ).order_by(Reminder.is_completed.asc(), Reminder.created_at.desc()).all()

def get_recent_expenses(db: Session, limit: int = 5) -> List[Expense]:
    return db.query(Expense).order_by(Expense.date.desc(), Expense.created_at.desc()).limit(limit).all()

def get_dashboard_notifications(db: Session) -> List[Dict[str, Any]]:
    today_val = date.today()
    notifications = []

    # 1. Overdue Bills (High 🔴)
    overdue_bills = db.query(Bill).filter(Bill.is_paid == False, Bill.due_date < today_val).all()
    ...
    due_tomorrow_bills = db.query(Bill).filter(Bill.is_paid == False, Bill.due_date == today_val + timedelta(days=1)).all()
    ...
    today_reminders = db.query(Reminder).filter(Reminder.is_completed == False, Reminder.date == today_val).all()
    ...
    tomorrow_reminders = db.query(Reminder).filter(Reminder.is_completed == False, Reminder.date == today_val + timedelta(days=1)).all()
    ...
    active_groceries = db.query(func.count(Grocery.id)).filter(Grocery.is_completed == False).scalar() or 0
