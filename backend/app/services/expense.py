...
def get_expenses_summary(db: Session) -> Dict[str, float]:
    ...
    # Queries
    today_sum = db.query(func.sum(Expense.amount)).filter(Expense.date == today_val).scalar() or Decimal('0.00')
    week_sum = db.query(func.sum(Expense.amount)).filter(Expense.date >= start_of_week).scalar() or Decimal('0.00')
    month_sum = db.query(func.sum(Expense.amount)).filter(Expense.date >= start_of_month).scalar() or Decimal('0.00')
    total_sum = db.query(func.sum(Expense.amount)).scalar() or Decimal('0.00')
